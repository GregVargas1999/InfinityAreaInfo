...
def press(button=LEFT):
    ...
    if _last_click["time"] is not None and datetime.datetime.now() - _last_click["time"] < datetime.timedelta(
            seconds=0.3) and _last_click["button"] == button and _last_click["position"] == location:
        # Repeated Click
...
def release(button=LEFT):
    ...
    if _last_click["time"] is not None and _last_click["time"] > datetime.datetime.now() - datetime.timedelta(
            microseconds=300000) and _last_click["button"] == button and _last_click["position"] == location:
        # Repeated Click
        Quartz.CGEventSetIntegerValueField(
            e,
            Quartz.kCGMouseEventClickState,
